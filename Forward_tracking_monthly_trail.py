...
def MR_yearly_source(source_lat,source_lon):
    global main, Evap, montly_precipip_tracking
    montly_precipip_tracking = np.zeroes((12,360,720))
    montly_precipip_tracking[:] = np.nan
    #global main
    main = 0
    ...
    for i in (range(12)):
        ...
        #    print('skipped')
            ...
            # save this  
            montly_precipip_tracking[i,:,:] = (source.fillna(0)*Evap) #shape of (source.fillna(0)*Evap) should be 360,720 
        MR.close()
        source.close()
...
for i in tqdm(range(Screened_data.shape[0])):
    ...
    if np.isnan(Evap) == True:
        ...
        ### Thre should be a code heare to sum all evaporation for pixels
        for i in range(12):
            total_montly_precipip_tracking[i,] +=  montly_precipip_tracking[i,] 
