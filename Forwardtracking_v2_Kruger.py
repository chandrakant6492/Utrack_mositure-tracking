...
"""
data = xr.open_dataset('/home/kruger/data/utrack/input/MODIS/Afric_TropcForest.nc').htrop_forest_20km
lon = data.longitude
lon = lon.values
...
for lat in range(data.latitude.shape[0]):
    for lon in range(data.longitude.shape[0]):
        if data[lat, lon].values == 1:
            lat_.append(round(float(data[lat,lon].latitude.values),2))
            lon_.append(round(float(data[lat,lon].longitude.values),2))
            Screened_data.append(data[lat,lon].values)
        else:
            continue
Screened_data = pd.DataFrame({'Lat': lat_,'Lon': lon_})
...
def MR_yearly_sink(source_lat, source_lon):
    ...
    for i in (range(12)):
        ...
        Evap_footprint_monthly[i] = (Evap.values*source)
...
"""
print(Evap_footprint_annual_sum.sum())
print(Evap_footprint_monthly_sum.sum())
...
Evap_footprint_annual_sum.to_netcdf('/home/chandra/data/Kruger/Backtracking/Forwardtracking_Evap_footprint_annual_sum_(Kruger_data).nc')
Evap_footprint_monthly_sum.to_netcdf('/home/chandra/data/Kruger/Backtracking/Forwardtracking_Evap_footprint_monthly_sum_(Kruger_data).nc')
